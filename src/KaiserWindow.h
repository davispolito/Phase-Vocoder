...
public:
    ...
    static void buildWindow( std::vector< float > & win, float shape );
    ...
    //! \param      shape is the Kaiser shaping parameter, controlling
    //!             the sidelobe rejection level.
    static void buildTimeDerivativeWindow( std::vector< float > & win, float shape );    
    ...
    static unsigned long computeLength( float width, float alpha );
